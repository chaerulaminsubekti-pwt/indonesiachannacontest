<footer class="bg-[#0A0A0A] text-gray-300 mt-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
                ...
                <p class="text-sm leading-relaxed">{{ $settings->tagline ?? 'Indonesia Channa Contest — wadah resmi penyelenggaraan kontes dan lomba di seluruh Indonesia.' }}</p>
            </div>
                ...
                <ul class="space-y-2 text-sm">
                    @if ($settings?->email_kontak)
                        <li class="flex items-center gap-2">
                            ...
                            <a href="mailto:{{ $settings->email_kontak }}" class="hover:text-[#FF1A1A] transition">{{ $settings->email_kontak }}</a>
                        </li>
                    @endif
                    @if ($settings?->no_wa_kontak)
                        <li class="flex items-center gap-2">
                            ...
                            <a href="https://wa.me/{{ $settings->no_wa_kontak }}" target="_blank" rel="noopener" class="hover:text-[#FF1A1A] transition">{{ $settings->no_wa_kontak }}</a>
                        </li>
                    @endif
                    <li>
                        <div class="flex gap-3 mt-3">
                            @if ($settings?->link_instagram)
                                <a href="{{ $settings->link_instagram }}" target="_blank" rel="noopener" class="hover:text-[#FF1A1A] transition" title="Instagram">
                            ...
                            @if ($settings?->link_facebook)
                                <a href="{{ $settings->link_facebook }}" target="_blank" rel="noopener" class="hover:text-[#FF1A1A] transition" title="Facebook">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                </a>
                            @endif
                            @if ($settings?->link_youtube)
                                <a href="{{ $settings->link_youtube }}" target="_blank" rel="noopener" class="hover:text-[#FF1A1A] transition" title="YouTube">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                                </a>
                            @endif
                            @if ($settings?->link_tiktok)
                                <a href="{{ $settings->link_tiktok }}" target="_blank" rel="noopener" class="hover:text-[#FF1A1A] transition" title="TikTok">
    ...
    <div class="border-t border-gray-800 py-4 text-center text-sm text-gray-500">
        &copy; {{ date('Y') }} {{ $settings->teks_copyright_footer ?? ($settings->nama_website ?? 'ICC') . ' — ' . ($settings->tagline ?? 'Indonesia Channa Contest') }}. All rights reserved.
    </div>
</footer>