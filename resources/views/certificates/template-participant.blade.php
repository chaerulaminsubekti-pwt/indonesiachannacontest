...
<body>
    <div class="o">
        <div class="i">
            ...
            <p class="e1">{{ $participant->event->nama_event }}</p>
            <div class="d1"></div>

            <p class="t1">Sertifikat Peserta</p>
            <div class="td1"></div>

            <p class="w1">{{ $participant->nama_pemilik }}</p>

            <div class="bw"><span class="b1">{{ $participant->class?->nama_kelas ?? 'Peserta' }}</span></div>

            <p class="x1">
                Telah terdaftar dan berpartisipasi pada Kelas <span class="h1">{{ $participant->class?->nama_kelas ?? '-' }}</span><br>
                dalam event <strong>{{ $participant->event->nama_event }}</strong><br>
                {{ \Carbon\Carbon::parse($participant->event->tanggal_mulai)->isoFormat('D MMMM Y') }} &middot; {{ $participant->event->venue }}, {{ $participant->event->wilayah_kota }}
            </p>
            ...
            <table class="ft1">
                <tr>
                    ...
                    <td class="sc">
                        <p>Penyelenggara</p>
                        <div class="sl"></div>
                        <p class="sn">{{ $participant->event->organizer?->user?->name ?? $participant->event->organizer?->nama_organisasi ?? '____________________' }}</p>
                    </td>
                </tr>
            </table>
